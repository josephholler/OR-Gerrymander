...
**Note:** To scale for the whole United States, or any state for which it is important to calculate multiple minority groups, it may be wiser to simplify the calculation and avoid double-counting people by focusing on people identifying as only one race.
...
### District Compactness and CDiff

Join convex hull estimates to Districts with block group estimates.

Calculate Cdiff scores

- `chDiff` is the District composition - the Convex Hull composition
- `ChAbsDiff` is the absolute value of chDiff
...
Calculate local gerrymandering scores by...

- Join District-level chDiff data to the fragments
...
- `gerry` is a local estimate of wasted minority votes due to gerrymandering. 

EXPLAIN gerry
CONFIRM the code still works when using the gerry_type variable
...
- are the geometries valid?  QGIS didn't think so, but st_make_valid has been applied?
